...
pub trait ConfigFile: Sized + Default + Clone {
        ...
        fn get_config() -> Self {
                if Self::override_fields() {
                        ...
                        default.merge(deserialize_custom_config::<Self::Raw>(
                                Self::get_type().as_default_filename().as_str(),
                        ))
                } else {
                        deserialize_config_or_default::<Self::Raw, Self>(
                                Self::get_type().as_default_filename().as_str(),
                        )
                }
        }
...
fn deserialize_to_config<R>(file_path: &Path) -> Result<R, AppError>
where
        R: DeserializeOwned,
{
        ...
        R: DeserializeOwned,
{
        match search_config_directories(file_name) {
                Some(file_path) => match deserialize_to_config::<R>(&file_path) {
                        Ok(s) => {
...
/// # Returns
/// The parsed configuration or an error if the file cannot be parsed.
fn deserialize_to_config_into<R, S>(file_path: &Path) -> Result<S, AppError>
where
...
pub fn deserialize_config_or_default<R, S>(file_name: &str) -> S
where
        R: DeserializeOwned + Into<S>,
        S: std::default::Default,
{
        ...
        match search_config_directories(file_name) {
                Some(file_path) => match deserialize_to_config_into::<R, S>(&file_path) {
                        Ok(s) => {
                                tracing::info!("Loaded config from {}", file_path.display());
                                s
                        }
