...
async fn get_command(client_id: i32) -> bool {
    ...
    match commands[0] {
        ...
        "sm" => {
            ...
            match functions::send_message(commands[1].parse().unwrap(), 0, None, None, None, text, client_id).await {
...
async fn handle_update(update: Update, auth_tx: &Sender<AuthorizationState>) {
    match update {
        ...
        Update::User(x) => eprintln!("[HANDLE UPDATE]: {:?} {:?}", x.user.usernames, x.user.id),
        ...
        // Update::ChatLastMessage(_) => eprintln!("[HANDLE UPDATE]: ChatLastMessage"),
        // Update::ChatPosition(_) => eprintln!("[HANDLE UPDATE]: ChatPosition"),
        // Update::ChatReadInbox(_) => eprintln!("[HANDLE UPDATE]: ChatReadInbox"),
        // Update::ChatReadOutbox(_) => eprintln!("[HANDLE UPDATE]: ChatReadOutbox"),
        // Update::ChatUnreadMentionCount(_) => eprintln!("[HANDLE UPDATE]: ChatUnreadMentionCount"),
        // Update::MessageMentionRead(_) => eprintln!("[HANDLE UPDATE]: MessageMentionRead"),
        // Update::ChatReplyMarkup(_) => eprintln!("[HANDLE UPDATE]: ChatReplyMarkup"),
        // Update::ChatDraftMessage(_) => eprintln!("[HANDLE UPDATE]: ChatDraftMessage"),
        // Update::ChatPermissions(_) => eprintln!("[HANDLE UPDATE]: ChatPermissions"),
        // Update::ChatNotificationSettings(_) => eprintln!("[HANDLE UPDATE]: ChatNotificationSettings"),
        // Update::ChatDefaultDisableNotification(_) => eprintln!("[HANDLE UPDATE]: ChatDefaultDisableNotification"),
        // Update::ChatIsMarkedAsUnread(_) => eprintln!("[HANDLE UPDATE]: ChatIsMarkedAsUnread"),
        // Update::ChatBlockList(_) => eprintln!("[HANDLE UPDATE]: ChatBlockList"),
        // Update::ChatHasScheduledMessages(_) => eprintln!("[HANDLE UPDATE]: ChatHasScheduledMessages"),
        // Update::UserFullInfo(_) => eprintln!("[HANDLE UPDATE]: UserFullInfo"),
        // Update::BasicGroupFullInfo(_) => eprintln!("[HANDLE UPDATE]: BasicGroupFullInfo"),
        // Update::SupergroupFullInfo(_) => eprintln!("[HANDLE UPDATE]: SupergroupFullInfo"),
        // Too much prints
    ...
    while let Some(state) = auth_rx.recv().await {
        match state {
            ...
            AuthorizationState::WaitPhoneNumber => loop {
                ...
                let response = functions::set_authentication_phone_number(input, None, client_id).await;
                match response {
                    Ok(_) => break,
            ...
            AuthorizationState::WaitOtherDeviceConfirmation(x) => {
                println!("Please confirm this login link on another device: {}", x.link);
            }
...
async fn main() {
    ...
    .await
    {
        Err(error) => eprintln!("[ERROR] \"functions::get_text_entitie\": {error:?}"),
